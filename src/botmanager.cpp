	...
	while (true)
	{
		...
		if (flatbufferData.size > 4)
		{
			...
			if (lasttime != gametickpacket->gameInfo()->secondsElapsed())
			{
				...
				// The fieldinfo or ball prediction might not have been set up by the framework yet.
				if (fieldInfoData.size > 4 && ballPredictionData.size > 4)
				{
					...
					int status = Interface::SetBotInput(bot->GetOutput(gametickpacket, fieldinfo, ballprediction), bot->index);
