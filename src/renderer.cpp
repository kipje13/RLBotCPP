...
void Renderer::DrawLine3D(Color color, rlbot::flat::Vector3 start, rlbot::flat::Vector3 end)
{
	...
	renderMessages.push_back(rlbot::flat::CreateRenderMessage(flatBufferBuilder, rlbot::flat::RenderType_DrawLine3D, coloroffset, &start, &end));
}

void Renderer::DrawPolyLine3D(Color color, std::vector<const rlbot::flat::Vector3*> points)
{
	auto colorbuilder = rlbot::flat::ColorBuilder(flatBufferBuilder);
	colorbuilder.add_a(color.a);
	colorbuilder.add_r(color.r);
	...
	{
		renderMessages.push_back(rlbot::flat::CreateRenderMessage(flatBufferBuilder, rlbot::flat::RenderType_DrawLine3D, coloroffset, points[i], points[i + 1]));
	}
}
